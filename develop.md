...
- `OUTBOUND_NO_PROXY` 里必须带 `pdf2zh`：app 提交翻译也走那份读 `HTTP_PROXY` 的 httpx，少了它这一次调用会被发到境外代理，代理解析不了 docker 主机名而返回 `502`——现象是 app 日志只有一句 `pdf2zh 提交失败 (HTTP 502)`，而 pdf2zh 容器连一条请求日志都没有（2026-09-19 实测；`.env.*.example` 已补上，老部署的 `.env` 要手动加）
- `paper_translations` 的写入必须显式 `conn.commit()`：`_get_connection()` 只负责 `close()`，而 psycopg 关闭连接会回滚未提交的事务，所以少一句 commit 就会「INSERT ... RETURNING 拿到了行、但库里一行都没有」，前端永远轮询到 `idle`（2026-09-19 发现并修复，`tests/test_pdf_translation.py::test_translation_writes_commit` 守住）
- Celery 的结果后端由 `worker.py` 在 fork 前显式设成 broker 的 Redis：镜像里的 `config.json` 固化了 `"CELERY_RESULT": null`，而 `ConfigManager.get` 只要 key 已存在就原样返回、不回退到 `pdf2zh/backend.py` 传入的默认值，于是 `result_backend=None`、Celery 用 `DisabledBackend`，`/v1/translate/<id>` 每次轮询都 `AttributeError` → Flask 500（2026-09-19 发现并修复，`docker-images.yml` 的冒烟测试断言了这个值）

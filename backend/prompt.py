...
- 若正文给出公式，解释公式中与结论直接相关的变量和作用；若材料没有公式，不得根据通行做法自行补公式。
- 有全文时应覆盖方法、训练/构建阶段、推理阶段、实验设置、主结果和局限。不要只围绕摘要重复。报告通常写成 4000 至 7000 个中文字符，但不要为了凑字重复。

1. 这篇论文要解决什么任务？
回答标准：先用一句话说清任务，再给出具体、可信的输入和输出样例；说明训练/辅助数据与测试目标数据之间的关系，让没有接触过计算机科学的人也能理解。任务设定必须附正文证据锚点。
